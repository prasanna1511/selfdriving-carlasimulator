...
class GlobalPlanner:
    ...
    def integrate_traffic_sign(self, traffic_sign_position: np.array, traffic_sign_type: str):
        ...
        max_speed_splits = traffic_sign_type.split("_")
        print("max_speed", max_speed_splits)
        
        if max_speed_splits[0] in ["left", "right"]:
            closest_nodes = self._find_closest_edge_nodes(location)
            adjacent_edges = self._get_adjacent_edges(closest_nodes)
            
            if max_speed_splits[0] == "left":
                for edge in adjacent_edges:
                    self._update_edge_cost(edge[0], edge[1], 1000)
                # print("left")   
            elif max_speed_splits[0] == "right":
                for edge in adjacent_edges:
                    self._update_edge_cost(edge[0], edge[1], 1000)
                # print("right")
                    
            elif max_speed_splits[0] in ["max"]:
            ...
            elif max_speed_splits[0] in ["left_turn", "right_turn"]:
                closest_nodes = self._find_closest_edge_nodes(location)
                ...
                if closest_nodes:
                    ...
                    for edges in adjacent_edges:
                        self._update_edge_cost(edges[0], edges[1], 1000)
                        print("edges", edges)
                        
                        if max_speed_splits[0] == "left_turn":
                            for edges in adjacent_edges["left"]:
                                self._update_edge_cost(edges[0], edges[1], 1)
                                
                        elif max_speed_splits[0] =="right_turn":
                            for edges in adjacent_edges["right"]:
                                self._update_edge_cost(edges[0], edges[1], 1)

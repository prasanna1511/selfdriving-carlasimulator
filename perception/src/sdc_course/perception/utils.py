...
def image_to_world(world, point: np.array, distance: float) -> np.array:
    ...
    K = world.get_vehicle().get_camera().calibration
    ...
    point_in_camera = np.dot(K_inv, image)
    # print("camera", point_in_camera)
    
    point_camera = s * point_in_camera
    # print("point_camera", point_camera)
    point_camera_Hom = np.array([point_camera[2], point_camera[0], - point_camera[1], 1])
    # print(point_camera_Hom)
    
    
    T_cam_from_world = transform_to_numpy(world.get_vehicle().get_camera().get_transform())
    # print(T_cam_from_world)
    
    point_world = np.dot(T_cam_from_world, point_camera_Hom)
